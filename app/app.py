...
# [X] On the homepage of the app, provide an area where users can post text messages. 
# [X] Users can click a button to upvote each post via WebSockets which can be seen by all users without a refresh
    ...
    if 'username' in session:
        if request.method == 'POST':
            ...
            if file and allowed_file(file.filename):
                # Randomize filename to prevent overrides of same image names
                filename = secrets.token_urlsafe(16) + '.' + file.filename.rsplit('.', 1)[1].lower()
